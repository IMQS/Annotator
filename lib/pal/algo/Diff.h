...
NOTE: This code was copied from https://github.com/bmharper/xo/blob/wip-rx-diff/xo/VirtualDom/Diff.cpp.
Since Ben Harper wrote that code, he is entitled to do whatever he wants with it, including copying
and using it here.
*/
class IMQS_PAL_API DiffCore {
public:
...
// Diff traits for 'char'
struct IMQS_PAL_API CharTraits {
...
struct IMQS_PAL_API ByteTraits {
	uint32_t Hash(uint8_t c) const {
		return (uint32_t) c;
	}
