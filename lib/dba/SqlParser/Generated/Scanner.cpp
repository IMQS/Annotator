...
#include "pch.h"
#if _MSC_VER >= 1400
#pragma warning(disable: 4996 6011 6386)
#endif
...
wchar_t* coco_string_create(const wchar_t *value, int startIndex)
{
	...
	if (value) {
		valueLen =  static_cast<int>(wcslen(value));
...
wchar_t* coco_string_create_append(const wchar_t* data1, const wchar_t* data2) 
{
	...
	if (data1) { data1Len = static_cast<int>(wcslen(data1)); }
	if (data2) {data2Len = static_cast<int>(wcslen(data2)); }
...
{
	int dataLen = static_cast<int>(wcslen(data));
	int endLen = static_cast<int>(wcslen(end));
...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable: 6031) // _setmode return value ignored
#endif
...
void Buffer::SetPos(int value) 
{
	...
	if ((value >= bufStart) && (value < (bufStart + bufLen))) { // already in buffer
		...
		bufLen =  static_cast<int>(fread(buf,  static_cast<int>(sizeof(unsigned char)),  static_cast<int>(bufCapacity), stream));
...
int Buffer::ReadNextStreamChunk() 
{
	...
	int read =  static_cast<int>(fread(buf + bufLen,  static_cast<int>(sizeof(unsigned char)), static_cast<int>(free), stream));
	if (read > 0) {
...
{
	buffer = new Buffer(buf,  static_cast<int>(len));
	Init();
}
...
Scanner::Scanner(const char* str) 
{
	
	buffer = new Buffer((unsigned char*)str,  static_cast<int>(strlen(str)));
	Init();
}
...
void Scanner::NewQuery(const char* buf) 
{
	if (buffer !=0)
	delete buffer;
	buffer = new Buffer((unsigned char*)buf,  static_cast<int>(strlen(buf)));
	Init();
...
void Scanner::Init() 
{
	...
	keywords.set(L"insert", 7);
	keywords.set(L"into", 8);
	keywords.set(L"dummy_where", 9);
	keywords.set(L"values", 13);
	keywords.set(L"or", 14);
	keywords.set(L"case", 15);
	keywords.set(L"and", 16);
	keywords.set(L"like", 21);
	keywords.set(L"is", 22);
	keywords.set(L"not", 23);
	keywords.set(L"isnull", 24);
	keywords.set(L"notnull", 25);
	keywords.set(L"null", 26);
	keywords.set(L"in", 27);
	keywords.set(L"cast", 38);
	keywords.set(L"true", 39);
	keywords.set(L"false", 40);
	keywords.set(L"as", 42);
	keywords.set(L"else", 43);
	keywords.set(L"end", 44);
	keywords.set(L"when", 45);
	keywords.set(L"then", 46);
...
Token* Scanner::NextToken() 
{
	...
	switch (state) {
		...
		case 4:
			...
			if ((ch >= L'0' && ch <= L'9') || ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_4;}
		...
		case 7:
			...
			{t->kind = 3; break;}
		case 8:
			if (ch == L'_' || (ch >= L'a' && ch <= L'z')) {AddCh(); goto case_9;}
			else {goto case_0;}
		case 9:
			case_9:
			if (ch <= 9 || (ch >= 11 && ch <= 12) || (ch >= 14 && ch <= L'Z') || (ch >= L'^' && ch <= 65535)) {AddCh(); goto case_9;}
		...
		case 14:
			...
			else if (ch == L'x') {AddCh(); goto case_13;}
			else {t->kind = 1; break;}
		case 15:
			case_15:
