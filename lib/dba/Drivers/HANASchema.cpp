...
Error HANASchemaReader::ReadSchema(uint32_t readFlags, Executor* ex, std::string tableSpace, schema::DB& db, const std::vector<std::string>* restrictTables) {
...
Error HANASchemaWriter::DropTable(Executor* ex, std::string tableSpace, const std::string& table) {
	auto s = ex->Sql();
	s.Fmt("DROP TABLE %v", MakeIdent(s, tableSpace, table));
	return ex->Exec(s);
}

Error HANASchemaWriter::CreateTable(Executor* ex, std::string tableSpace, const schema::Table& table) {
	if (table.IsView() || table.IsMaterializedView())
		return Error("Views are not implemented in HANASchemaWriter");

	auto s = ex->Sql();
	s.Fmt("CREATE COLUMN TABLE %v (", MakeIdent(s, tableSpace, table.GetName()));
...
Error HANASchemaWriter::CreateIndex(Executor* ex, std::string tableSpace, const std::string& table, const schema::Index& idx) {
	auto s = ex->Sql();
	...
	std::string idxName = MakeIdent(s, tableSpace, "idx_" + table + "_" + fields);

	s.Clear();
	s.Fmt("CREATE %v INDEX %v ON %v (", idx.IsUnique ? "UNIQUE" : "", idxName, MakeIdent(s, tableSpace, table));
	for (const auto& f : idx.Fields) {
		s.Identifier(f, true);
		s += ",";
	}
...
std::string HANASchemaWriter::MakeIdent(SqlStr& s, const std::string& tableSpace, const std::string table) {
	SqlStr tmp(s.Dialect);
	if (tableSpace != "")
		tmp.Fmt("%Q.%Q", tableSpace, table);
	else
		tmp.Fmt("%Q", table);
	return tmp.Str;
}
